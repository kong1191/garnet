...
static sysmgr::ServiceProviderPtr service_provider;
...
long port_create(const char *path, uint32_t num_recv_bufs,
  ...
  if (startup_context == nullptr) {
    ...
    fuchsia::sys::ConnectToEnvironmentService(service_provider.NewRequest(),
                                              "trusty_service");
  }

  std::string service_name(path);
  bool is_existed;
  service_provider->LookupService(
      service_name, [&is_existed](bool found) { is_existed = found; });
  bool ret = service_provider.WaitForResponse();
  if (ret) {
    FXL_LOG(ERROR) << "Internal error on calling LookupService";
    return ERR_GENERIC;
  }
  ...
  service_provider->AddService(service_name, [port](zx::channel channel) {
    port->Bind(std::move(channel));
  });
