// Copyright 2018 Open Trust Group
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include <fbl/auto_lock.h>
#include <lib/zx/vmar.h>
#include <lib/zx/vmo.h>
#include <zircon/types.h>

#include "lib/ree_agent/cpp/channel.h"

namespace ree_agent {

zx_status_t TipcChannelImpl::Create(uint32_t num_items,
                                    size_t item_size,
                                    fbl::unique_ptr<TipcChannelImpl>* out) {
  auto chan = fbl::unique_ptr<TipcChannelImpl>(new TipcChannelImpl());
  if (!chan) {
    return ZX_ERR_NO_MEMORY;
  }

  fbl::AutoLock lock(&chan->msg_list_lock_);
  for (uint32_t i = 0; i < num_items; i++) {
    auto item = fbl::make_unique<MessageItem>(i);
    if (!item) {
      return ZX_ERR_NO_MEMORY;
    }

    zx_status_t status = item->InitNew(item_size);
    if (status != ZX_OK) {
      return status;
    }

    chan->free_list_.push_back(fbl::move(item));
  }

  chan->num_items_ = num_items;
  *out = fbl::move(chan);
  return ZX_OK;
}

zx_status_t TipcChannelImpl::BindPeerInterfaceHandle(
    fidl::InterfaceHandle<TipcChannel> handle) {
  peer_.Bind(std::move(handle));

  fidl::VectorPtr<zx::vmo> shared_buffers;
  peer_->RequestSharedBuffers(&shared_buffers);

  if (!shared_buffers) {
    return ZX_ERR_NO_MEMORY;
  }

  uint32_t msg_id = 0;
  for (auto& vmo : *shared_buffers) {
    auto item = fbl::make_unique<MessageItem>(msg_id++);
    if (!item) {
      return ZX_ERR_NO_MEMORY;
    }

    zx_status_t status = item->InitFromVmo(fbl::move(vmo));
    if (status != ZX_OK) {
      return status;
    }

    peer_items_.push_back(fbl::move(item));
  }

  return ZX_OK;
}

void TipcChannelImpl::Close() {}

void TipcChannelImpl::RequestSharedBuffers(
    RequestSharedBuffersCallback callback) {
  fbl::AutoLock lock(&msg_list_lock_);
  fidl::VectorPtr<zx::vmo> shared_buffers;

  for (const auto& item : free_list_) {
    zx::vmo shared_buffer = item.GetDuplicateVmo();
    shared_buffers.push_back(std::move(shared_buffer));
  }

  callback(std::move(shared_buffers));
}

void TipcChannelImpl::GetFreeMessage(GetFreeMessageCallback callback) {
  fbl::AutoLock lock(&msg_list_lock_);

  if (free_list_.is_empty()) {
    callback(Status::NO_MEMORY, 0);
    return;
  }

  auto item = free_list_.pop_front();

  uint32_t msg_id = item->msg_id();
  outgoing_list_.push_back(fbl::move(item));

  callback(Status::OK, msg_id);
}

void TipcChannelImpl::SendFilledMessage(uint32_t msg_id,
                                        uint64_t filled_size,
                                        SendFilledMessageCallback callback) {
  fbl::AutoLock lock(&msg_list_lock_);

  auto iter = outgoing_list_.find_if(
      [&msg_id](const MessageItem& item) { return item.msg_id() == msg_id; });
  if (iter == outgoing_list_.end()) {
    callback(Status::NOT_FOUND);
    return;
  }

  auto item = outgoing_list_.erase(iter);
  item->update_filled_size(filled_size);
  filled_list_.push_back(fbl::move(item));

  callback(Status::OK);
}

Status TipcChannelImpl::SendMessage(void* msg, size_t msg_size) {
  FXL_DCHECK(msg != nullptr);
  fbl::AutoLock lock(&msg_list_lock_);

  uint32_t msg_id;
  Status status;

  peer_->GetFreeMessage(&status, &msg_id);
  if (status != Status::OK) {
    return status;
  }

  auto item = peer_items_[msg_id].get();
  if (msg_size > item->size()) {
    return Status::NO_MEMORY;
  }

  void* buffer_ptr = item->PtrFromOffset(0);
  memcpy(buffer_ptr, msg, msg_size);

  peer_->SendFilledMessage(msg_id, msg_size, &status);
  return status;
}

Status TipcChannelImpl::GetMessage(uint32_t* msg_id, size_t* len) {
  FXL_DCHECK(msg_id != nullptr);
  FXL_DCHECK(len != nullptr);
  fbl::AutoLock lock(&msg_list_lock_);

  if (filled_list_.is_empty()) {
    return Status::NO_MSG;
  }

  auto item = filled_list_.pop_front();

  *msg_id = item->msg_id();
  *len = item->filled_size();
  read_list_.push_back(fbl::move(item));

  return Status::OK;
}

Status TipcChannelImpl::ReadMessage(uint32_t msg_id,
                                    uint32_t offset,
                                    void* buf,
                                    size_t* buf_size) {
  FXL_DCHECK(buf != nullptr);
  FXL_DCHECK(buf_size != nullptr);
  fbl::AutoLock lock(&msg_list_lock_);

  auto it = read_list_.find_if(
      [&msg_id](const MessageItem& item) { return msg_id == item.msg_id(); });

  if (it == read_list_.end()) {
    return Status::NOT_FOUND;
  }

  size_t filled_size = it->filled_size();
  if (offset > filled_size) {
    return Status::OUT_OF_RANGE;
  }

  size_t bytes_to_copy = filled_size - offset;
  if (*buf_size < bytes_to_copy) {
    return Status::NO_MEMORY;
  }

  memcpy(buf, it->PtrFromOffset(offset), bytes_to_copy);
  *buf_size = bytes_to_copy;

  return Status::OK;
}

Status TipcChannelImpl::PutMessage(uint32_t msg_id) {
  fbl::AutoLock lock(&msg_list_lock_);

  auto it = read_list_.find_if(
      [&msg_id](const MessageItem& item) { return msg_id == item.msg_id(); });

  if (it == read_list_.end()) {
    return Status::NOT_FOUND;
  }

  auto item = read_list_.erase(it);
  free_list_.push_back(fbl::move(item));

  return Status::OK;
}

}  // namespace ree_agent
