...
class MessageItem
 ...
 public:
  ...
  static fbl::unique_ptr<MessageItem> FromTransport(
      SharedMessageItem transport) {
    auto item = fbl::make_unique<MessageItem>(transport.msg_id);
    if (!item) {
      return nullptr;
    }

    if (item->InitFromVmo(fbl::move(transport.vmo)) != ZX_OK) {
      return nullptr;
    }
