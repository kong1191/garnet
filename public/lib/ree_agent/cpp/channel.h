...
class TipcChannelImpl : public TipcChannel {
 public:
  ...
  bool initialized() {
    return binding_.is_bound() && peer_.is_bound() &&
           (peer_items_.size() == num_items_);
  }

 protected:
  void Close() override;
  void RequestSharedBuffers(RequestSharedBuffersCallback callback) override;
  void GetFreeMessage(GetFreeMessageCallback callback) override;
  void SendFilledMessage(uint32_t msg_id,
                         uint64_t filled_size,
                         SendFilledMessageCallback callback) override;
 ...
 private:
  ...
  fidl::SynchronousInterfacePtr<TipcChannel> peer_;
  std::vector<fbl::unique_ptr<MessageItem>> peer_items_;
  size_t num_items_;
