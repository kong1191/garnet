...
class TipcObject : public fbl::RefCounted<TipcObject> {
 public:
  static constexpr zx_signals_t NEED_ATTENSION = ZX_USER_SIGNAL_0;
  ...
  zx_status_t AddParent(TipcObject* obj);
  void RemoveParent(uint32_t parent_id);
 ...
 private:
  ...
  zx::event event_;
  zx_signals_t event_state();

  fbl::Mutex mutex_;
  zx_signals_t event_state_ FXL_GUARDED_BY(mutex_);
