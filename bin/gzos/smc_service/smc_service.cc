...
zx_status_t SmcService::CreateNopThreads() {
  ...
  fbl::AutoLock lock(&nop_thread_lock_);
  nop_stop_.store(false);

  uint32_t cpu;
  for (cpu = 0; cpu < kMaxNopThreads; cpu++) {
    ...
    int ret = thrd_create_with_name(&nop_thread_[cpu], thread_entry, &args,
                                    name_buffer.c_str());
    if (ret != thrd_success) {
...
void SmcService::JoinNopThreads() {
  ...
  if (nop_stop_.exchange(true)) {
    return;
  }

  zx_smc_cancel_nop(GetHandle());

  uint32_t i;
  for (i = 0; i < kMaxNopThreads; i++) {
    if (nop_thread_[i]) {
      thrd_join(nop_thread_[i], nullptr);
    }
  }
}
