...
class SmcService {
 public:
  static SmcService* Create(zx_handle_t smc, zx_handle_t vmo);
 ...
 private:
  ...
  async_wait_result_t OnSmcReady(
      async_t* async, zx_status_t status,
      const zx_packet_signal_t* signal);
  void OnSmcClosed(async_t* async, zx_status_t status, const char* action);

  mutable fbl::Mutex lock_;
  zx_handle_t smc_handle_;
  async::Wait smc_wait_;
