...
static constexpr const uint32_t kMaxNopThreads = 4;
...
class SmcService {
 public:
  ...
  SmcService()
      : smc_handle_(ZX_HANDLE_INVALID), shared_mem_(nullptr), nop_stop_(true) {}
  ...
  bool nop_stop() { return nop_stop_.load(); }
 ...
 private:
  ...
  fbl::Mutex nop_thread_lock_;
  thrd_t nop_thread_[kMaxNopThreads] __TA_GUARDED(nop_thread_lock_);
  fbl::atomic<bool> nop_stop_;
};
