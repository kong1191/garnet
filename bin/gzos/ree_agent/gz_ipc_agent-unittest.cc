    ...
    switch (ctrl_hdr->type) {
      case CtrlMessageType::CONNECT_RESPONSE: {
        ...
        if (it != waiter_list_.end()) {
          ...
          memcpy(waiter->result(), body, sizeof(gz_ipc_conn_req_body));
          waiter->Signal();
        }
        break;
      }
...
TEST_F(GzIpcAgentTest, EchoTest) {
  ...
  std::string test_string = "echo_test";
  echo->EchoString(test_string, [&test_string](std::string response) {
