...
TEST_F(TipcEndpointTableTest, Lookup) {
  TipcEndpoint* ep[NUM_TEST_CHANNEL];
  uint32_t i;

  for (i = 0; i < NUM_TEST_CHANNEL; i++) {
    ep[i] = ep_table_->LookupByAddr(dst[i]);
    ASSERT_TRUE(ep[i] != nullptr);
    EXPECT_EQ(ep[i]->src_addr, src[i]);
    EXPECT_EQ(ep[i]->channel->cookie(), ep[i]);
  }
}
...
TEST_F(TipcEndpointTableTest, FindInUse) {
  ...
  for (i = 0; i < NUM_TEST_CHANNEL; i++) {
    ep[i] = ep_table_->FindInUseSlot(slot_id);
    ASSERT_TRUE(ep[i] != nullptr);
    EXPECT_EQ(ep[i]->src_addr, src[0]);

    if (i > 0) {
      EXPECT_EQ(ep[i], ep[i - 1]);
    }
  }
  ...
  for (i = 0; i < NUM_TEST_CHANNEL; i++) {
    ...
    if (i > 0) {
      EXPECT_NE(ep[i], ep[i - 1]);
    }
  }

  // free first endpoint slot
  ep_table_->FreeSlotByAddr(dst[0]);

  // start finding in-use endpoint from slot 0
  slot_id = 0;
  for (i = 0; i < NUM_TEST_CHANNEL - 1; i++) {
    ep[i] = ep_table_->FindInUseSlot(slot_id);
    ASSERT_TRUE(ep[i] != nullptr);
    EXPECT_EQ(ep[i]->src_addr, src[i + 1]);
    EXPECT_EQ(slot_id, i + 1);
    slot_id++;
  }

  // find again and no in-use endpoint was found
...
TEST_F(TipcAgentTest, DisconnectAll) {
  ...
  // no in-use endpoint in endpoint table
  uint32_t slot = 0;
