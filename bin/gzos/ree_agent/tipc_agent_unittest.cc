...
class TaServiceFake : public AppmgrServices {
 public:
  static constexpr uint32_t kNumItems = 3u;
  static constexpr size_t kItemSize = 1024u;
  static char port_name[kTipcMaxServerNameLength];
...
char TaServiceFake::port_name[kTipcMaxServerNameLength] = "test.svc1";
...
class TipcAgentTest : public ::testing::Test {
 ...
 protected:
  ...
  zx_status_t SendConnectRequest(void* buf, uint32_t src, uint32_t dst,
                                 const char* name) {
    auto hdr = reinterpret_cast<tipc_hdr*>(buf);
...
TEST_F(TipcAgentTest, PortConnectOk) {
  ...
  uint32_t src_addr = 5;
  ASSERT_EQ(SendConnectRequest(buf, src_addr, kTipcCtrlAddress,
                               TaServiceFake::port_name),
            ZX_OK);
...
TEST_F(TipcAgentTest, PortConnectWithInvalidName) {
  ...
  ASSERT_EQ(SendConnectRequest(buf, src_addr, kTipcCtrlAddress, invalid_name),
            ZX_OK);
