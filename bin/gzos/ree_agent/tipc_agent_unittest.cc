...
TipcPortImpl port_service(kNumItems, kItemSize);

const char port_name[] = "test.svc1";

void ConnectToTaService(zx::channel ch, const std::string& service_name) {
  if (service_name.compare(port_name) == 0) {
    fidl::InterfaceRequest<ree_agent::TipcPort> request(std::move(ch));
    port_service.Bind(std::move(request));
  }
}
...
TEST_F(TipcAgentTest, PortConnectOk) {
  ...
  uint32_t src_addr = 5;
  ASSERT_EQ(SendConnectRequest(buf, src_addr, kTipcCtrlAddress, port_name),
            ZX_OK);
