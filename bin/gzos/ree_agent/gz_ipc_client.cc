...
zx_status_t GzIpcClient::HandleFreeVmo(gz_ipc_ctrl_msg_hdr* ctrl_hdr) {
  ...
  auto rec = LookupSharedMemoryRecord(body->koid);
  if (rec) {
    rec->reset_vmo();
  }
...
zx_status_t GzIpcClient::AllocSharedMemory(size_t size, zx::vmo* vmo_out) {
  ...
  auto free_mem =
      fbl::MakeAutoCall([this, phys_base] { alloc_->Free(phys_base); });
  ...
  rec->set_release_handler([this, koid, phys_base] {
    alloc_->Free(phys_base);
    RemoveSharedMemoryRecord(koid);
  });

  InstallSharedMemoryRecord(koid, std::move(rec));
