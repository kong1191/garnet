...
#define VerifyWaitOne(object, expected_event)                          \
  {                                                                    \
    WaitResult result;                                                 \
    EXPECT_EQ(object->Wait(&result, zx::time() + zx::usec(1)), ZX_OK); \
...
#define VerifyWaitAny(expected_id, expected_event)                       \
  {                                                                      \
    WaitResult result;                                                   \
    EXPECT_EQ(obj_mgr_->Wait(&result, zx::time() + zx::usec(1)), ZX_OK); \
    EXPECT_EQ(result.handle_id, expected_id);                            \
    EXPECT_EQ(result.event, expected_event);                             \
  }
...
TEST_F(TipcObjectTest, WaitAny) {
  ...
  WaitResult result;
  EXPECT_EQ(obj_mgr_->Wait(&result, zx::time() + zx::usec(1)),
            ZX_ERR_TIMED_OUT);
}
...
TEST_F(TipcObjectSetTest, StackedObjectSet) {
  ...
  // Wait from object_set1 and verify events
  ...
  // Wait from object_set2 should not only get object2 but also object_set1
  VerifyWaitObjectSet(object_set2_, object2_->handle_id(), TipcEvent::MSG);
  VerifyWaitObjectSet(object_set2_, object_set1_->handle_id(),
                      TipcEvent::READY);
  ...
  WaitResult result;
  EXPECT_EQ(object_set1_->Wait(&result, zx::time() + zx::usec(1)),
            ZX_ERR_TIMED_OUT);
  EXPECT_EQ(object_set2_->Wait(&result, zx::time() + zx::usec(1)),
            ZX_ERR_TIMED_OUT);
}
