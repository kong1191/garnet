...
zx_status_t GzIpcAgent::HandleMessage(void* buf, size_t size) {
  ...
  if (size < sizeof(gz_ipc_msg_hdr)) {
    FXL_LOG(ERROR) << "message buffer size too small, size=" << size;
    return ZX_ERR_INVALID_ARGS;
  }

  auto hdr = reinterpret_cast<gz_ipc_msg_hdr*>(buf);

  if (size != (sizeof(gz_ipc_msg_hdr) + hdr->len)) {
...
zx_status_t GzIpcAgent::HandleCtrlMessage(gz_ipc_msg_hdr* hdr) {
  FXL_DCHECK(hdr);

  auto ctrl_msg = reinterpret_cast<gz_ipc_ctrl_msg_hdr*>(hdr->data);
  auto msg_body = reinterpret_cast<void*>(ctrl_msg + 1);

  switch (ctrl_msg->type) {
    case CtrlMessageType::CONNECT_REQUEST:
      if (ctrl_msg->body_len != sizeof(gz_ipc_conn_req_body)) {
        break;
      }
      return HandleConnectRequest(hdr->src, msg_body);

    case CtrlMessageType::DISCONNECT_REQUEST:
      if (ctrl_msg->body_len != sizeof(gz_ipc_disc_req_body)) {
        break;
      }
...
zx_status_t GzIpcAgent::HandleConnectRequest(uint32_t remote_addr, void* req) {
  ...
  auto send_reply_msg =
      fbl::MakeAutoCall([this, &status, &remote_addr, &local_addr]() {
        ...
        FXL_CHECK(SendMessageToRee(kCtrlEndpointAddress, remote_addr, &res,
                                   sizeof(res)) == ZX_OK);
      });
  ...
  fbl::AutoLock lock(&lock_);
  ep_table_[local_addr]->reader().set_error_handler(
      [this, local_addr, remote_addr] {
        ...
        FXL_CHECK(SendMessageToRee(local_addr, kCtrlEndpointAddress, &disc_req,
                                   sizeof(disc_req)) == ZX_OK);
      });

  auto conn_req = reinterpret_cast<gz_ipc_conn_req_body*>(req);
...
zx_status_t GzIpcAgent::HandleDisconnectRequest(uint32_t src_addr, void* req) {
  ...
  auto disc_req = reinterpret_cast<gz_ipc_disc_req_body*>(req);
...
zx_status_t GzIpcAgent::HandleEndpointMessage(gz_ipc_msg_hdr* hdr) {
  FXL_DCHECK(hdr);
  ...
  FXL_LOG(ERROR) << "Endpoint addr " << hdr->src << " not found, msg dropped";
  return ZX_OK;
}
