  ...
  auto child = fbl::AdoptRef(new fs::Service([this, service_name](
    ...
    if (it != startup_services_.end()) {
      ...
      if (it == launched_apps_.end()) {
        auto app =
            std::make_unique<DynamicServiceAppInstance>(root_, this, app_name);
        FXL_CHECK(app);
...
DynamicServiceAppInstance::DynamicServiceAppInstance(
    fbl::RefPtr<fs::PseudoDir> root, DynamicServiceApp* app,
    std::string app_name)
    : root_(root), app_(app), app_name_(app_name) {
...
void DynamicServiceAppInstance::BindServiceProvider(
    fidl::InterfaceRequest<fuchsia::sys::ServiceProvider> request) {
  service_provider_.AddBinding(std::move(request));
}

void DynamicServiceAppInstance::LookupService(
...
void DynamicServiceAppInstance::AddService(fidl::StringPtr service_name) {
  app_->AddService(app_name_, service_name, [this, &service_name]() {
    registered_services_.push_back(service_name);
  });
}

void DynamicServiceAppInstance::WaitOnService(fidl::StringPtr service_name,
                                              WaitOnServiceCallback callback) {
  app_->WaitOnService(app_name_, service_name, std::move(callback));
}

void DynamicServiceAppInstance::CancelWaitOnService(
    fidl::StringPtr service_name) {
  app_->CancelWaitOnService(app_name_, service_name);
}

void DynamicServiceAppInstance::RemoveService(fidl::StringPtr service_name) {
...
void DynamicServiceAppInstance::RemoveAllServices() {
