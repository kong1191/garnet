...
class DynamicServiceAppInstance : public ServiceRegistry {
 public:
  DynamicServiceAppInstance(fbl::RefPtr<fs::PseudoDir> root,
                            DynamicServiceApp* app, std::string app_name);
  DynamicServiceAppInstance() = delete;
 ...
 private:
  ...
  FXL_DISALLOW_COPY_AND_ASSIGN(DynamicServiceAppInstance);
};
...
class DynamicServiceApp {
 ...
 private:
  ...
  std::map<std::string, std::unique_ptr<DynamicServiceAppInstance>>
      launched_apps_;
