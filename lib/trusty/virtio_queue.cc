...
static bool validate_queue_range(trusty::SharedMem *shared_mem,
                                 zx_vaddr_t addr,
                                 size_t size) {
  uintptr_t mem_addr = shared_mem->addr();
  size_t mem_size = shared_mem->size();
...
static void queue_set_segment_addr(VirtioQueue* queue,
                                   ...
                                   size_t size,
                                   T** ptr) {
  // TODO(sy): confirm the guest paddr is under the range of shared memory
  // provided by Secure Memory object, and translate it to host vaddr
  zx_vaddr_t host_vaddr = (uintptr_t)nullptr;

  *ptr = validate_queue_range(queue->shared_mem(), host_vaddr, size)
...
zx_status_t VirtioQueue::ReadDesc(uint16_t desc_index, virtio_desc_t* out) {
  ...
  size_t mem_size = shared_mem_->size();
  ...
  // TODO(sy): confirm the desc.addr is under the range of shared memory
  // provided by Secure Memory object, and translate it to host vaddr
  out->addr = nullptr;
  out->len = desc.len;
...
zx_status_t VirtioQueue::HandleDescriptor(virtio_queue_fn_t handler,
  ...
  uint16_t head;
  uint32_t used_len = 0;
  uintptr_t mem_addr = shared_mem_->addr();
  size_t mem_size = shared_mem_->size();
