...
class LinuxFake {
 public:
  ...
  zx_status_t AllocVring(fw_rsc_vdev_vring* vring) {
    ...
    vring->da = shared_mem_->VirtToPhys(buf, size);
    return ZX_OK;
  }
...
class TipcDriverFake {
 public:
  ...
  zx_status_t AllocVring(machina::VirtioQueueFake* queue,
	...
	queue->Init(buf_, vring);
    return ZX_OK;
  }

  zx_status_t Probe(fw_rsc_vdev* vdev_descr) {
    zx_status_t status;

    status = AllocVring(&rx_queue_, &vdev_descr->vring[0]);
    if (status != ZX_OK)
      return status;

    status = AllocVring(&tx_queue_, &vdev_descr->vring[1]);
    if (status != ZX_OK)
      return status;

    vdev_descr->status = VIRTIO_STATUS_ACKNOWLEDGE | VIRTIO_STATUS_DRIVER |
                         VIRTIO_STATUS_DRIVER_OK;
    return ZX_OK;
  }
