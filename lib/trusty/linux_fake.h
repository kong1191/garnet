...
class TipcDriverFake {
 public:
  ...
  zx_status_t AddRxBuffer(void* buf, uint16_t buf_size) {
    uint16_t idx;
    zx_status_t status =
        rx_queue_.BuildDescriptor().AppendWriteable(buf, buf_size).Build(&idx);

    if (status == ZX_OK) {
      fbl::AllocChecker ac;
      rx_bufs_.push_back(idx, &ac);
      if (!ac.check()) {
  ...
  zx_status_t GetRxBuffer(virtio_desc_t* desc) {
    if (rx_bufs_.is_empty()) {
      return ZX_ERR_SHOULD_WAIT;
    }

    uint16_t idx = rx_bufs_.erase(0);
    return rx_queue_.queue()->ReadDesc(idx, desc);
  }
